...
async function add(_, { issue }) {
  ...
  validateIssue(_, { issue: validIssue });

  validIssue.created = new Date();
  validIssue.id = await getNextSequence('issues');

  if (validIssue.status === undefined) {
    validIssue.status = 'New';
  }

  const result = await db.collection('issues').insertOne(validIssue);
...
async function update(_, { id, changes }) {
  ...
  const hasUpdatableFields = changes && (
    changes.title || changes.status || changes.owner
    || changes.effort || changes.due || changes.description
  );
